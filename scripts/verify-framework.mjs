...
async function main() {
  ...
  const extendedWorkspaceSource = await readFile(join(process.cwd(), 'src', 'features', 'managers', 'extended', 'ExtendedManagerWorkspace.tsx'), 'utf-8')
  const healthCenterSource = await readFile(join(process.cwd(), 'src', 'features', 'health', 'HealthCenter.tsx'), 'utf-8')
  ...
  assert(hubSource.includes('MANAGER_WORKSPACE_GROUPS') && hubSource.includes('workspaceGroups.map') && hubSource.includes('Unified workspaces'), 'manager hub renders grouped ecosystem workspaces from the shared map')
  ...
  assert(healthCenterSource.includes("from './riskGovernance'") && healthCenterSource.includes('RiskGovernanceOverview') && !healthCenterSource.includes('Risk governance cockpit') && riskGovernanceOverviewSource.includes('Risk governance cockpit') && ['releaseRiskProfile', 'dependencyDiff', 'auditEvidence', 'vulnerabilityRemediationPlan', 'licenseReport', 'registryReport', 'credentialRotationPlan', 'readinessReport', 'reportArtifactIndex'].every((signal) => riskGovernanceOverviewSource.includes(signal)) && riskGovernanceOverviewSource.includes('aggregateStatus') && riskGovernanceOverviewSource.includes('riskStatus'), 'health center risk governance overview is isolated in a risk submodule with complete release, dependency, vulnerability, license, registry, credential, readiness, and artifact status contracts')
  ...
  assert(healthCenterSource.includes('CiIntegrationPlanPanel') && healthCenterSource.includes('exportCiIntegrationPlan') && healthCenterSource.includes('GitHub Actions') && !healthCenterSource.includes('Deployment release gates') && ciIntegrationPlanPanelSource.includes('CI integration plan') && ciIntegrationPlanPanelSource.includes('Deployment release gates') && ciIntegrationPlanPanelSource.includes('deploymentWarningCount'), 'health center exposes CI integration plan summary, deployment gates, and workflow export controls through the automation details module')
