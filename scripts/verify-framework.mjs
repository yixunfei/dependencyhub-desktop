...
async function main() {
  ...
  assert(['mcp', 'skills', 'ai-agents', 'a2a'].every((id) => MANAGER_DEFINITIONS.find((manager) => manager.id === id)?.route === '/ai'), 'AI dependency managers route to the dedicated AI workspace')
  ...
  const layoutMenuSource = await readFile(join(process.cwd(), 'src', 'components', 'Layout', 'layoutMenu.tsx'), 'utf-8')
  ...
  assert(layoutMenuSource.includes('MANAGER_WORKSPACE_GROUPS') && layoutMenuSource.includes('groupedManagerIds') && layoutMenuSource.includes('buildManagerMenuEntries'), 'sidebar menu entries are generated from the shared manager workspace group map in the layout menu module')
  assert(layoutSource.includes('findWorkspaceGroupByPath') && layoutSource.includes('buildManagerMenuEntries') && layoutSource.includes('buildMainMenuEntries'), 'main layout resolves the active workspace group and renders the sidebar from the layout menu module')
