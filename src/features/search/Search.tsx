...
type SearchType = 'npm' | 'pip' | 'maven' | 'cargo' | 'gradle' | 'go' | 'flutter' | 'native'
...
const SEARCH_TYPE_OPTIONS = [
  ...
  { label: 'Flutter pub', value: 'flutter' },
...
const SEARCH_PLACEHOLDERS: Partial<Record<SearchType, string>> = {
  ...
  flutter: '搜索 pub.dev 包，例如 dio、provider'
}
        ...
        <Spin spinning={loading}>
          {results.length === 0 ? (
            ...
            <Table
              ...
              scroll={{ x: searchType === 'go' ? 1080 : searchType === 'npm' ? 1180 : searchType === 'pip' || searchType === 'flutter' ? 900 : searchType === 'maven' || searchType === 'gradle' ? 1040 : 960 }}
            />
          )}
        </Spin>
      </div>
...
async function searchPackages(type: SearchType, query: string, cwd: string, coordinateOptions?: CoordinateSearchSettings): Promise<SearchItem[]> {
  ...
  if (type === 'flutter') {
    const result = await window.electronAPI.flutter.search(query)
    return uniqueByKey(result.map((pkg: FlutterSearchResult) => ({
      type,
      key: pkg.name,
      name: pkg.name,
      version: pkg.version || '',
...
async function fetchVersions(item: SearchItem): Promise<string[]> {
  ...
  if (item.type === 'flutter') {
    return await window.electronAPI.flutter.versions(item.name)
  }
...
function resolvePackageUrl(item: SearchItem): string {
  ...
  if (item.type === 'flutter') {
    return `https://pub.dev/packages/${encodeURIComponent(item.name)}`
  }
